    ...
    <div class="container-cupom mt-5">
        ...
        <div class="row border-bottom border-dark mb-3">
            <div class="col-12 border border-dark mb-3">
                ...
                <?php if($cliente->cnpj_cpf != ""){ ?>
                <h4 class="mt-2 text-black-50"><strong><?= $cliente->cnpj_cpf ?></strong></h3>
                <?php } ?>
                <?php if($cliente->endereco != ""){ ?>
                <h4 class="mt-1">
                    <?php echo $cliente->endereco . ", " . $cliente->numero . " - " . $cliente->bairro; ?></h4>
                <?php } ?>
                <?php if($cliente->cep != "" || $cliente->nome_cidade != ""){ ?>
                <h4 class="mt-1 mb-3">
                <?php
                    if($cliente->cep != ""){ 
                        echo $cliente->cep; 
                    }
                    if($cliente->cep != "" && $cliente->nome_cidade != ""){ 
                        echo " - "; 
                    }
                    if($cliente->nome_cidade != ""){ 
                        echo $cliente->nome_cidade . "/" . $cliente->uf; 
                    }  
                ?>
                </h4>
                <?php } ?> 
        ...
        <?php } ?>
        <div class="row border-bottom border-dark mb-3">
            <table class="table text-uppercase mb-3 h5">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">R$ <?= number_format((float) ($produto->valor_unitario), 2, ",", ".") ?></td>
                        <td class="text-center"><?= number_format((float) ($produto->quant_pedida), 3, ",", ".") ?></td>
                        <td class="text-right">R$
                            <?= number_format((float) ($produto->valor_unitario * $produto->quant_pedida), 2, ",", ".") ?></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
                    ...
                    <table class="table table-borderless h4 mb-4 mt">
                        <tbody>
                            <tr>
                                ...
                                <td class="text-right"><strong>R$ <?= number_format((float) ($total), 2, ",", ".") ?></strong>
                                </td>
                            </tr>
