...
class EngenhariaController extends CI_Controller {
    ...
    public function editarEstruturaProduto($CodProduto){
        ...
        $listaCoproduto = $this->engenharia->getCoprodutoPorEstrutura($CodProduto);
        $listaProdutoCons = $this->produto->getProdutoEstruturaComponente($CodProduto, $listaComponente);
        $listaProdutoCop = $this->produto->getProdutoEstruturaCoproduto($CodProduto, $listaCoproduto);
        ...
        if($listaEstrutura == null){
            ...
            $dados = array(
                ...
                'lista_coproduto' => $listaCoproduto,
                'lista_produto_cons' => $listaProdutoCons,
                'lista_produto_cop' => $listaProdutoCop,
                'menu' => 'Cadastro'
            );        
    ...
    public function inserirEstruturaCoproduto(){

        $this->form_validation->set_rules('CodProdutoCop', 'Coproduto', 'required',
                    array('required' => 'Você deve preencher o campo %s'));
        $this->form_validation->set_rules('QuantCoproduto', 'Quantidade do Coproduto', 'required|callback_more_zero',
                    array('required' => 'Você deve preencher o campo %s'));
        ...
        if($this->form_validation->run() == false){
            ...
            $CodProduto = $this->input->post('CodProdudoProd');
            $this->editarEstruturaProduto($CodProduto);
            redirect(base_url("estrutura-produto/editar-estrutura-produto/{$CodProduto}#coproduto"), "home", "refresh");
            ...
            $dados = [
                'id_empresa' => getDadosUsuarioLogado()['id_empresa'],
                'cod_produto'  => $CodProduto,
                'cod_coproduto' => $this->input->post('CodProdutoCop'),
                'quant_coproduto' => str_replace(",",".",(str_replace(".","",$this->input->post('QuantCoproduto'))))
            ];

            $erro = $this->engenharia->insertEstruturaCoproduto($dados);

            if ($erro['code'] == null){
                $this->session->set_flashdata('sucesso', 'Coproduto cadastrado com sucesso'); 
    ...
    public function salvarEstruturaCoproduto(){

        $CodProduto = $this->uri->segment(3);
        $SeqCoproduto = $this->uri->segment(4);

        //Validações dos campo e array das mensagens apresentadas
        $this->form_validation->set_rules('QuantCoprodutoEdit', 'Quantidade do Coproduto', 'required|callback_more_zero',
            array('required' => 'Você deve preencher o campo %s'));
        ...
        if($this->form_validation->run() == false){
            ...
            $dataCop = [
                'quant_coproduto' => str_replace(",",".",(str_replace(".","",$this->input->post('QuantCoprodutoEdit'))))
            ];

            $erro = $this->engenharia->updateEstruturaCoproduto($SeqCoproduto, $dataCop);

            if ($erro['code'] == null){
                $this->session->set_flashdata('sucesso', 'Coproduto alterado com sucesso');
    ...
    public function excluirEstruturaCoproduto()
    {

        $SeqEstruturaCoproduto = $this->input->post("excluir_todos_coproduto");
        $numRegs = count($SeqEstruturaCoproduto);

        if($numRegs > 0){
            
            $erro = $this->engenharia->deleteEstruturaCoproduto($SeqEstruturaCoproduto);
        ...
        $CodProdutoProd = $this->input->post('CodProdudoProd');
        redirect(base_url("estrutura-produto/editar-estrutura-produto/{$CodProdutoProd}#coproduto"), "home", "refresh");
    } 
