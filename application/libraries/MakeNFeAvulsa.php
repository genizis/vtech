 ...
 */
class MakeNFeAvulsa extends CommonNFe
{
    private $_CI;
    private $makeNFe;
    private $nota;
    ...
    public function __construct($params)
    {
        ...
        $this->nota = $this->_CI->faturamentoNotaFiscal->getByIdAvulsa($params['id']);
        $faturamentoId = $this->nota->cod_faturamento_pedido;
        $this->nota_fiscal = $this->_CI->fiscal->getNotaFiscalporCodigo($faturamentoId);
        $this->naturezaOperacao = $this->_CI->naturezaOperacao->getById($this->nota->tb_fis_natureza_operacao_id);
        $this->destinatario = $this->_CI->faturamentoNotaFiscal->getClienteByFaturamentoIdAvulsa($this->nota->cod_faturamento_pedido);
        $this->produtos = $this->_CI->faturamentoNotaFiscalItem->getNFeItensNFAvulsa($params['id']);
        $this->transportador = $this->_CI->transportador->getTransportadorById($this->nota_fiscal->cod_transportador);
    ...
    public function getInfNFe()
    {
        ...
        if($this->empresa->versao_nfe == "" || $this->empresa->versao_nfe == null){
            throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Versão de NF não definida. Reveja os dados da sua empresa");                    
        }
    ...
    public function getIde()
    {
        ...
        if($this->empresa->codigo_uf == "" || $this->empresa->codigo_uf == null){
            throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Sem cidade definida. Reveja os dados da sua empresa");                    
        }
        if($this->naturezaOperacao == null){
            throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Não há uma natureza de operação definida para o uso no Frente de Caixa. Reveja os dados da sua empresa");                    
        }
    ...
    public function getRefNFe()
    {
        ...
        if($this->nota->nf_referencia != null){

            $stdRefNFe = new stdClass();
            $stdRefNFe->refNFe = $this->nota->nf_referencia;

            $this->makeNFe->tagrefNFe($stdRefNFe);
            return $this->makeNFe;
        }        
    }
        ...
        if($this->destinatario != null){
            ...
            if($this->destinatario->cep != "" && $this->destinatario->cep != null){
                ...
                if($this->destinatario->endereco == "" || $this->destinatario->endereco == null){
                    throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Cliente sem endereço definido");
                }elseif($this->destinatario->bairro == "" || $this->destinatario->bairro == null){
                    throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Cliente sem bairro definido");
                }elseif($this->destinatario->codigo_municipio == 0 || $this->destinatario->codigo_municipio == null){
                    throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Cliente sem cidade definido");
                }
...
//        $this->totais['totalTributoNacional'] = 0;
        try{
            ...
            foreach ($this->produtos as $key => $orderItem) {
                ...
                if($orderItem->cod_ncm == "" || $orderItem->cod_ncm == null){
                    throw new InvalidArgumentException("<br>(MakeNFeAvulsa) Produto (" . $orderItem->cod_produto . ") " . $orderItem->nome_produto . " não possui NCM definido");                    
                }

                $percentProduto = $orderItem->valor_fat_item / $this->nota_fiscal->valor_total;
    ...
    public function getFaturamento()
    {
        ...
        $total = 0;
//        if ($this->nota->getTbFatNotaFiscalFaturamentos()->count() > 0) {
//            foreach ($this->nota->getTbFatNotaFiscalFaturamentos() as $faturamento) {
//                $total += (float)$faturamento->getValor();
//            }
//            $codigoNFeTpag = $this->nota->getTbComPedido()->getTbFinFormaPagamento()->getCodigoNFeTPag();
//            $detPag->tPag = sprintf('%02d', (null !== $codigoNFeTpag) ? $codigoNFeTpag : 15);//Boleto
//        }
        $detPag->vPag = $total;

        $this->makeNFe->tagdetPag($detPag);

        //Verifica se não existe parcelamentos então retorna xml
//        if ($this->nota->getTbFatNotaFiscalFaturamentos()->count() === 0) {
        return $this->makeNFe;
//        }
        ...
        //Grupo Fatura Y02
//        $sdtFat = new stdClass();
//        $sdtFat->nFat = sprintf('%06d', $this->nota->getNumero());
//        $sdtFat->vOrig = $total;
//        $sdtFat->vDesc = null;
//        $sdtFat->vLiq = $total;
//
...
//
//        foreach ($this->nota->getTbFatNotaFiscalFaturamentos() as $key => $faturamento) {
//            $sdtFatDup = new stdClass();
//            $sdtFatDup->nDup = sprintf('%03d', ($key + 1));
//            $sdtFatDup->dVenc = $faturamento->getVencimento()->format('Y-m-d');
//            $sdtFatDup->vDup = $faturamento->getValor();
...
//        return $this->makeNFe;
    ...
    {
        $tipoFrete = 0;
        if($this->nota_fiscal->tipo_frete == 2){
            $tipoFrete = 1;
        }
