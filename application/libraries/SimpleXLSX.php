	...
	private function _unzip( $filename, $is_data = false ) {
		...
		if ( $is_data ) {
			...
			if ( ! is_readable( $filename ) ) {
				$this->error( 1, 'Arquivo não encontrado ' . $filename );
		...
		*/
		...
		$aEL = count( $aE );
		if ( $aEL === 0 ) {
			$this->error( 2, 'Formato de arquivo desconhecido' );
...
//				file_put_contents( basename( $name ), $entry_xml ); // @to do comment!!!
			...
			if ( $e ) {
				$this->error( 3, 'XML de entrada ' . $name . ' erro ' . $e->message . ' Linha ' . $e->line );
			}
		} else {
			$this->error( 4, 'XML de entrada não encontrado ' . $name );
		}
	...
	public function getEntryData( $name ) {
		...
		$this->error( 5, 'Entrada não encontrada ' . ( $dir ? $dir . '/' : '' ) . $name );
...
//						$this->hyperlinks[ $worksheetIndex ][ $ref ] = (string) $hyperlink['display'];
		...
		$this->error( 6, 'Planilha não encontrada ' . $worksheetIndex );
